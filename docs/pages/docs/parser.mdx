...
- `command` - the raw command line (`string`) or the command-line arguments (`string[]`)
...
- `command` - `process.env['COMP_LINE']` or `process.argv.slice(2)`
- `config.termWidth` - `process.stdout.columns` or `process.stderr.columns`
- `config.compIndex` - `Number(process.env['COMP_POINT'])` or `undefined`
...
`OptionValues<typeof _your_options_>`) to see how the object is structured.

This method returns a promise that can be awaited in order to await any
[async callback](#asynchronous-parsing).

## Option validation

The term _validation_ here means verifying the sanity of an option's definition. It is intended for
development and testing, and should _not_ be used in production, in order to improve the performance
of your application. The validations behave like assertions: they assert that your options will work
as expected when delivered to end users.

To validate a set of option definitions, call the `validate` method on the parser instance. The
following sections describe the various kinds of validation performed. In most of them, the option
key specified in the definitions object is included in the error message.
...
An option name must not contain whitespace or the equals sign `=`, since this character may be used
as option-value separator on the command-line. Any other character (including Unicode code points)
is allowed. Empty strings and `null`s are ignored. A flag option's
...
strings and `null`s are ignored. A flag option's [negation names](options.mdx#negation-names) and
the [positional marker](options.mdx#positional--marker) are included in the validation.
...
niladic option, _if_ it includes a required value that is neither `undefined` nor `null`.
...
data types from one invocation to another (a promise and a non-promise). To support these use cases,
the library checks the return type of each invocation to know if it is a promise or not, and act
accordingly.

There are four possible combinations of sequenced invocations:

1. `non-promise` followed by `non-promise`
2. `non-promise` followed by `promise`
3. `promise` followed by `promise`
4. `promise` followed by `non-promise`
...
- The first case is handled simply by appending the normalized elements to the previous value, since
...
Note that, once a promise is returned from a parsing callback, there's no turning back to a
non-promise.
...
callback in its definition, the parsing loop might exit before calling it and the option value will
remain an empty array (non-promise). If using TypeScript this will be evident when trying to await
the option value.
...
must then await each value in this object that is a promise, in order to use the value.
...
syntactic sugar for creating a bare result object into which the values will be parsed. But unlike
its sibling, `parseAsync` returns a promise, so you must await it in order to use the option values.
