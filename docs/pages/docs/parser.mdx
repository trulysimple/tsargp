...
> parameters. In this case, the parser must initialize the option value to an empty array. This
> means that, even if an option declares an async parsing > callback in its definition, the parser
> might exit the parsing loop before calling the callback and therefore the option value will remain
> an empty array (non-promise). If using TypeScript this will be evident when trying to await the
> option value.
...
The completion algorithm implemented by the library is explained below. In what follows, _arg_ is
the word being completed, i.e., the command-line argument ending at the completion index (it may be
an empty string):

1. If _arg_ is expected to be an option name:
   - If _arg_ is a known option name, that name is returned; else
   - If there's no positional option, the available option names are filtered by the prefix _arg_,
     and then returned; else
   - _arg_ is treated as a parameter of the positional option; go to step 2
2. _arg_ is the parameter of a non-niladic option:
   - If the option has a [complete callback](options.mdx#complete-callback), the result of the
     callback is returned; else
   - If it is a boolean option, the words `'true'{:ts}` and `'false'{:ts}` are filtered by the
     prefix _arg_, and then returned; else
   - If the option has enumerated values, those values are converted to string, filtered by the
     prefix _arg_, and then returned; else
   - If it is a variadic array option, and was not specified with a positional marker, the available
     option names are filtered by the prefix _arg_ and returned; else
   - No words are returned, and the default bash completion (if enabled with `-o`) is attempted.

During completion, help and version options are skipped, but others are processed as usual. This
means that completion callbacks can inspect the values parsed before it, in order to make better
...
During parsing, there are two situations where the parser may suggest option names:
...
- when it expects an option parameter and the current argument fails to be parsed, and the affected
  option is a variadic array option (but only if it was specified neither with a positional marker
  nor with an [inline value](#inline-values))
...
Both validation and parsing methods may throw errors, and the error messages can be styled. The
styles can be configured with an additional parameter to the parser constructor, which contains the
...
- `option` - the style of option names
- `url` - the style of URLs
