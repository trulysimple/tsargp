...
`'nearest'{:ts}` (round to nearest integer). This normalization is applied _before_ checking value
constraints.
...
The `separator` attribute, if present, specifies a delimiter to split parameter values. If
...
resulting from argument parsing. However, they do have side-effects, as explained below.
