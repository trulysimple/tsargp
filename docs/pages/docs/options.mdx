...
options allow the calling application or user to configure the command that will be run. **tsargp**
...
> NOTE: Since the library was designed to avoid using the console directly, it doesn't have a proper
> way to handle deprecated options specified on the command-line (e.g., by logging a warning). For
> this purpose, you may want to use a custom parsing callback (more on that later), although it is
> only available for non-niladic options.
...
> NOTE: When an object is specified, `undefined` values can be used to signify required presence,
> and `null` values to signify required absence.
...
> NOTE: If this value is not known beforehand (e.g., if it depends on the values of other options),
> it should be left undefined, so that you can detect whether it was specified on the command-line
...
> NOTE: If it contains an opening angle bracket (`<` character), it is printed as-is; otherwise it
...
make better suggestions for bash completion than the [default algorithm](parser.mdx) would. It
...
should call `ArgumentParser.parseAsync` and await its result. Mutually exclusive with
