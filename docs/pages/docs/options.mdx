...
| [boolean](#boolean-option)   | positional, single                   | `boolean{:ts}`  |                    |                     |
| [string](#string-option)     | positional, single                   | `string{:ts}`   | trim, case         | enums, regex        |
| [number](#number-option)     | positional, single                   | `number{:ts}`   | round              | enums, range        |
| [strings](#strings-option)   | positional, multi, delimited, append | `string[]{:ts}` | unique, trim, case | enums, regex, limit |
| [numbers](#numbers-option)   | positional, multi, delimited, append | `number[]{:ts}` | unique, round      | enums, range, limit |
...
[inline styles](styles.mdx#inline-styles). The `group` attribute is the name of a group under which
the option should appear. The `hide` attribute, if present, indicates to the formatter that the
option should not be displayed in the help message. The `deprecated` attribute is just syntactic
