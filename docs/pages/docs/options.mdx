...
[result object](parser.mdx#option-values). The meaning of other attributes is explained in the next
sections.
...
option-value separator). Empty names or `null{:ts}`s can be specified in order to skip the
...
The `link` attribute is a URL for an external resource or media, that is included in the help
...
options. Mutually exclusive with [default value](#default-value).
...
_delimited_, or _not variadic_). Can be a string or a regular expression. Mutually exclusive with
[parse callback](#parse-callback) and [parse delimited](#parse-delimited).
...
that it is _delimited_, or _not variadic_). If asynchronous, then the data type of the option value
in the resulting object will be enclosed in a promise. Mutually exclusive with
...
although it may have the [required](#required) or the [default value](#default-value) attribute for
a `boolean{:ts}` data type. In addition, it has the following attribute.
