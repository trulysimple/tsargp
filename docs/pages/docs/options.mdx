...
_monadic_ means that the option accepts a single parameter, and _variadic_ means multiple parameters
(including zero). The data type indicates the base type of the option value in the object resulting
from argument parsing. The meaning of the other attributes is explained in the next sections.
...
#### Description, group, hide & deprecated

These attributes only have effect in the formatting of the help message.

The `desc` attribute is the option synopsis, and may contain
[inline styles](formatter.mdx#inline-styles). The `group` attribute is the name of a group under
which the option should appear. The `hide` attribute, if present, indicates to the formatter that
the option should not be displayed in the help message. The `deprecated` attribute is just syntactic
sugar for the option deprecation reason, and may contain inline styles.
...
option. It has the following optional attributes:

- `names` - the style of the option names
- `param` - the style of the option paramater
- `desc` - the style of the option description
...
been previously specified must be reset by a new list of arguments.

#### Default value

The `default` attribute is the same as that of the set of [parameter attributes](#default-value)
for a `boolean{:ts}` data type.
