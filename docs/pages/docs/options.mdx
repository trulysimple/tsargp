...
Any command-line interface is composed of commands and options (sometimes called flags). The options
allow the calling application or user to configure the command that will be run. **tsargp** offers a
...
All option types share a set of attributes in addition to their type. From these, only the `names`
attribute is required, the rest is optional. They are described below.
...
"slot" in the help message "names" column (see the [formatter docs](./formatter) for details),
although there must be at least one non-empty name.
...
It is not validated and can be anything. If not specified, the first name in the `names` array will
be used.

#### Description, group, hide, styles

These attributes only have effect in the formatting of the help message.

The `desc` attribute is the option synopsis, and may contain inline [styles](./styles). The `group`
attribute is the name of a group under which the option should appear. The `hide` attribute, if
present, indicates to the formatter that the option should be excluded from the help message. The
`styles` attribute can be used to customize the styling of the help message columns (names,
parameter and description) for this option.

#### Deprecated

The `deprecated` attribute is just syntactic sugar for the option deprecation reason that appears in
the help message. Since the library was designed to avoid using the console directly, it doesn't
have a proper way to handle deprecated options specified on the command-line (e.g., by logging a
warning). For this purpose, you may want to use a custom parsing callback (more on that later),
although it is only available for non-niladic options.

#### Requires & required

The `requires` attribute, if present, specifies the option requirements. This can be either:
...
command-line. If an object, then every referenced option should be present and have the required
value. If an expression, then it is evaluated as follows:
...
The `default` attribute, if present, specifies the option default value. If this value is not known
beforehand (e.g., if it depends on the values of other options), it should be left undefined, so
that you can detect whether it was specified on the command line and take appropriate action.
...
column. If it contains an opening angle bracket (`<` character), it is printed as-is; otherwise it
is enclosed in angle brackets, like so: `<param_name>`. Mutually exclusive with
...
Additionally, this attribute accepts a string which is used as positional marker. In this case,
all arguments that appear _beyond the marker_ will be considered positional.
...
make better suggestions for bash completion than the default algorithm would (see the [parser
docs](./parser) for details). It accepts two parameters:
...
- `rest`: the remaining command-line arguments. The first element in this list is the one triggering
  the completion (which may be an empty string).

It returns the list of completion words. This callback should not throw errors. If asynchronous,
you should call `ArgumentParser.parseAsync` and await its result.
...
match. Any parameter that does _not match_ the regular expression will will cause an error to be
thrown. Mutually exclusive with [enumeration](#enumeration).
...
be within. Any parameter that is _not within_ the given range will will cause an error to be thrown.
You may want to use `[-Infinity, Infinity]` to disallow `NaN`. Mutually exclusive with
...
parameter that causes the option value to _exceed_ the given limit will will cause an error to be
thrown.
...
1. instantiate a [formatter](./formatter) class with the provided configuration
...
The `format` attribute, if present, specifies the [help format](./formatter) configuration. The
`headingStyle` attribute, if present, specifies a [style](./styles) to be applied to group headings.
...
The `version` attribute is a semantic version or version information.

#### Resolve

The `resolve` attribute is a resolution function scoped to the module where a `package.json` file
should be searched. For use in non-browser environments only. You should use [`import.meta.resolve`]
as value. This results in an asynchronous operation, so you should call `ArgumentParser.parseAsync`
...
The **function** option executes a callback function. The following attributes can be specified.
...
The `break` attribute, if present, indicates that the parser should exit the parsing loop
immediately after returning from the callback. This is useful for creating sub-commands, as
explained in the [sub-commands guide](/guides/sub-commands).
...
> NOTE: We could have reused the boolean option for this purpose and make the parameter optional,
> but it would complicate the parser implementation unnecessarily. Besides, the flag option has a
