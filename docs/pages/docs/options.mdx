...
make better suggestions for bash completion than the [default
algorithm](parser.mdx#completion-algorithm) would for the option. It accepts two parameters:
...
It should return the list of completion words and should not throw any error. If asynchronous, you
...
It should return a single value. If asynchronous, then the data type of the option value in the
resulting object will be enclosed in a promise. Mutually exclusive with [separator](#separator) and
...
an error to be thrown. You may want to use `[-Infinity, Infinity]{:ts}` to disallow `NaN{:ts}`.
...
These option types expect no parameter on the command-line and (with the notable exception of the
[flag option](#flag-option)) have no intrinsic value. Hence, they do not appear in the object
...
- `completing` - `true{:ts}` if performing completion (but not in the current iteration)

It should return no value (void). It can be asynchronous, in which case you should call
