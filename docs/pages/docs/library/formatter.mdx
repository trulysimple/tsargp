...
The `styles` property specifies the styles of text elements in the help message. It has the same
properties as the [error styles](validator.mdx#error-styles) and the same default values. They may
be overriden by an option's [display styles](options.mdx#display-styles), if present.
...
These phrases will be formatted according to the [text formatting](#text-formatting) rules.
...
In addition to item phrases, some description items such as the option synopsis or the deprecation
reason are texts that the formatter splits into words before adding to the option description. The
following formatting features are supported for these items.
