...
Help messages are styled with the same styles as [error messages](validator.mdx#error-styles),
except that they may get overriden by (or combined with) an option's
