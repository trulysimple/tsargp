...
are subject to the same set of constraints as that of the command-line arguments, which are listed
below.
...
An option must _not_ declare a requirement that includes its _own_ option key.

#### Unknown required option

An option must _not_ declare a requirement that includes an _unknown_ option key.

#### Niladic option required with a value

An option must _not_ declare a requirement that includes an option key referencing a niladic option,
_if_ it includes a required value that is neither `undefined{:ts}` nor `null{:ts}`.
...
validated at run-time. An option must _not_ declare a requirement that includes an option key
referencing a non-niladic option, _if_ it includes a required value that does _not_ conform to the
...
`ErrorConfig` object. It has some optional properties, as described below.

### Error styles

The `styles` property specifies the styles of error messages, and has the following optional
properties:
...
<Callout type="warning">
  These styles are _not_ used in the help option [format](options.mdx#format--heading-style)
...
- `missingRequiredOption` - raised by the parser when an option that is required was not specified
  (or vice-versa)
...
Error phrases may have specifiers that can be used to substitute for a value.
...
| optionRequires             | `%o` = the option name; `%t` = the option requirements                          |
| missingRequiredOption      | `%o` = the option name                                                          |
| missingParameter           | `%o` = the option name                                                          |
| missingPackageJson         |                                                                                 |
| positionalInlineValue      | `%o` = positional marker                                                        |
| optionInlineValue          | `%o` = the option name                                                          |
