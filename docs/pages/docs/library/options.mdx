...
`'round'{:ts}` (round to the nearest integer). This normalization is applied _before_ checking value
constraints.
...
value, so you can check whether it was processed or not by inspecting the resulting object.
