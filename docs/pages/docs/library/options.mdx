...
The **function** option executes a generic callback function and saves the returned value as the
option value. The following attributes can be specified.
...
<Callout type="info">
  ...
  exit the parsing loop immediately after returning from the callback.
</Callout>

#### Command options | callback

The `options` attribute is the set of option definitions for the command, and is also required. You
may instead specify a (non-asynchronous) callback that returns the option definitions. This allows
the implementation of [recursive commands](../guides/commands.mdx).
