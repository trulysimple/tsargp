...
| [function](#function-option) | niladic                                 | `number{:ts}`[^1]  |                    |                     |
| [command](#command-option)   | niladic                                 | `boolean{:ts}`[^2] |                    |                     |
...
[^2]: `true{:ts}` if command was called; `false{:ts}` otherwise.
...
<Callout type="default">None of these attributes is required, not even the `names`.</Callout>
...
respective "slot" in the help message [names column](formatter.mdx#names-column).
...
<Callout type="default">
  When an object is specified, `undefined{:ts}` values can be used to signify required presence,
  and `null{:ts}` values to signify required absence.
</Callout>
...
- `values` - the values parsed up until the current iteration
...
`'round'{:ts}` (round to nearest integer). This normalization is applied _before_ checking value
constraints.
...
[flag option](#flag-option), these options have _side-effects_, rather than having an intrinsic
value.

<Callout type="info">
  When bash completion is in effect, help and version options will not be processed.
</Callout>

### Help option

The **help** option is a convenient specialization of the [function option](#function-option) that
handles the formatting of a help message. Internally, it performs the following actions:
...
The application should catch the string and print it on a terminal.

<Callout type="warning">
  The help message contains control sequences that can only be interpreted by a terminal. It should
  not, for instance, be written to a file.
</Callout>
...
The **function** option executes a generic callback function and records the number of times that
the callback is executed in the option value. The following attributes can be specified.
...
<Callout type="warning">
  When setting this attribute, the requirements of all options specified up to the current option on
  the command-line (including itself) will be verified. Hence, you should make it clear in the help
...
handles the execution of "nested" commands. It lets you specify the command's options in one of its
attributes and parses the remaining arguments for you. It also records `true{:ts}` in the option
value, so you can check whether it was processed or not by inspecting the
[result object](parser.mdx#option-values).

<Callout type="default">
  As you might have guessed, this option allows the definition of hierarchical commands (but not
  recursive ones). This is explained in detail in the [commands guide](/docs/guides/commands).
</Callout>
...
<Callout type="info">
  When bash completion is in effect, the command callback will _not_ be called. Otherwise, the
  parser will exit the parsing loop immediately after returning from the callback.
</Callout>

#### Command options

The `options` attribute is the set of option definitions for the command, and is also required.

<Callout type="warning">
  All arguments specified after the affected option on the command-line will be parsed using the
  option definitions from this attribute, _not_ those in which the option was declared. Hence, you
  should make it clear in the help message that all arguments pertaining to the command should be
  specified after it.
</Callout>
...
[default value](#default-value--callback) attribute for a `boolean{:ts}` data type. In addition, it
