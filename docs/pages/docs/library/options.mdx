...
<Callout type="info">
  Since the library was designed to avoid using the console directly, it does not have a proper way
...
<Callout type="info">
  If this attribute contains an opening angle bracket `'<'{:ts}`, it is printed as-is; otherwise it
  is enclosed in angle brackets (e.g., `<param_name>`).
</Callout>
...
<Callout type="warning">
  When this attribute is set, the requirements of all options specified before the affected option
  on the command-line (including itself) will be analyzed. Hence, you should make it clear in the
  help message that any required option should be specified before it.
</Callout>
