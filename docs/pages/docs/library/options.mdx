...
| [command](#command-option)   | niladic                                 | `number{:ts}`[^1] |                    |                     |
...
[^1]: the number of times the function or command was called.
...
the callback was called in the option value. The following attributes can be specified.
...
<Callout type="info">
  The parser increments the option value by `1{:ts}` _after_ executing the callback.
</Callout>
...
<Callout type="info">
  The parser records the value `1{:ts}` in the option value _before_ executing the callback. This is
  to support counting of recursive calls, although the common case is to allow the application to
  check whether or not the the command was processed, by inspecting the resulting object.
</Callout>
...
The `options` attribute is the set of option definitions for the command, and is also required. You
may specify a non-asynchronous callback that returns the option definitions, and this allows the
