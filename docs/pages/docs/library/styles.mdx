...
- by the help message formatter to assemble description items from configured
  [description phrases](formatter.mdx#description-phrases)
- by the argument parser to create error messages based on configured
  [error phrases](validator.mdx#error-phrases)

In addition to description and error phrases, some description items such as the option synopsis or
the deprecation reason are texts that the help formatter splits into words before adding to the
option description. The formatting features supported for these items are described next.
