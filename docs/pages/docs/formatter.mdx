...
Some help items, such as the option synopsis and the deprecation reason, are texts that the
formatter splits into words and styles before adding them to the help entry's description column.
The following formatting features are supported for these items.
