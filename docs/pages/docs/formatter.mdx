...
the set of options that did not explicitly declare an
[option group](options.mdx#description-group-hide--deprecated) in their definition. The
`formatGroups` method, its sibling, returns a `Map{:ts}` of option group names to help messages
...
as was specified in the [names](options.mdx#names--preferred-name) attribute. Each option name
reserves a "slot" in the corresponding index in this column. For example, if the list contains the
names `'-f'{:ts}`, `'-ff'{:ts}`, `''{:ts}` and `'--flag'{:ts}`, the resulting entry might be
formatted as:
...
This column contains either of:
...
setting. If the width does not accommodate an entry's description column while respecting the
indentation level, it is instead wrapped relative to the beginning of each line.

## Help format

In addition to option definitions, the formatter constructor accepts a `HelpFormat` object that can
be used to customize the message format. This configuration will apply to all the help entries. It
...
The `desc` property specifies the level of indentation for the option description (in number of
...
The `descAbsolute` property, if true, indicates that the indentation level for the option
...
The `desc` property specifies the number of line breaks to insert before the option description. It
defaults to `0{:ts}`.
...
The `desc` property, if true, indicates that the option description should not be displayed. It
defaults to `false{:ts}`.
...
- `names` - the default style of the option names
- `param` - the default style of the option paramater
- `desc` - the default style of the option description
- `boolean` - the style of boolean values
- `string` - the style of string values
- `number` - the style of number values
- `regex` - the style of regular expressions
- `option` - the style of option names in the description (but not the option's own names)
- `text` - The style of general text

The first three of these properties may be overriden by the [option's styles](options.mdx#styles)
...
> embedded in user-provided texts) is rendered in bold intensity with yellow foreground color.

### Help items

The `items` property specifies the kind of items that will be displayed in the option description,
and in what order. It is an array whose values can be one of the following.

- `desc` - the user-provided option description
...
- `default` - the option default value, if any
- `deprecated` - reports if the option is deprecated

## User-provided text

The user-provided [text attributes](options.mdx#description-group-hide--deprecated) are split into
words and styles before being added to a help entry's description column. The following formatting
features are supported for these attributes.

### Inline styles

Texts can contain inline styles. The formatter will try its best to preserve styles while formatting
the help message and wrapping text in the option description. However, it might happen that some
corner case is not covered by the unit tests. If you find one, please submit a [bug report].

### Paragraphs and lists

Although markdown is not supported, paragraphs and itemized/numbered lists are formatted in a manner
akin to how they would be in markdown. For example, the following text
