...
can be used to customize the message format. This configuration will apply to all the help entries.
It contains a set of optional properties, as described below.
...
- `descr` - the default style of the option description
...
Both the [synopsis and the deprecation reason](options.mdx#synopsis--deprecated) are text attributes
that get split into words and styles before being added to a help entry's description column. The
following formatting features are supported for these attributes.
