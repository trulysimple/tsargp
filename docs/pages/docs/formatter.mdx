...
them accepting an optional parameter:
...
[option group](options.mdx#description-group-hide-styles--deprecated) in their definition. The
...
Each option definition will result in a help entry to be included in the message. An entry is
...
- `multivalued` - reports if an array option is multivalued
