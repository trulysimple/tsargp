...
<Callout type="default">
  [Function options] and [command options] accept any kind of value as the default. In the case of
  object values, it's useful to specify a custom `toString` method to render it in the help message:
  ```ts
  toString() {
...
- if asynchronous, you should await the result of [`parseAsync`]
...
The `format` attribute specifies a custom [help format] configuration.
...
- it results in an asynchronous operation, so you should await the result of [`parseAsync`]
...
- altering the option definitions in response to an option value
  - e.g., if `--verbose` is set, add more help items to the [help format]
