...
in effect (but not in the current iteration). The `isComp` property can be used to check whether any
of the remaining arguments is to be completed, when `comp` is true.
