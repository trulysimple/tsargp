...
There is a total of seven option types, as summarized in the table below:

| Type       | Attribute classes                                         |
| ---------- | --------------------------------------------------------- |
| [help]     | [basic], [message]                                        |
| [version]  | [basic], [message]                                        |
| [command]  | [basic], [value],                                         |
| [flag]     | [basic], [value], [environment]                           |
| [single]   | [basic], [value], [environment], [parameter], [selection] |
| [array]    | [basic], [value], [environment], [parameter], [selection] |
| [function] | [basic], [value], [environment], [parameter]              |
...
- **delimited** - single parameter in which values can be delimited with a [separator]
...
Non-niladic options can be configured with normalization and constraint attributes. There are three
kinds of attributes for this purpose:
...
accepted by that option, `undefined{:ts}` to signify _presence_ or `null{:ts}` to signify _absence_.
...
- `req.all` - an expression that is satisfied when _all_ requirements are satisfied
...
<Callout type="info">Can only be used on monadic or delimited options.</Callout>
...
<Callout type="info">Mutually exclusive with [append values].</Callout>
...
<Callout type="info">
  Mutually exclusive with [regular expression] and [numeric range]. Explicitly _not_ available for
  the [boolean option].
</Callout>
...
<Callout type="info">Mutually exclusive with [enumeration].</Callout>
...
Some attributes are shared by all valued options except command options. They are described below.
...
<Callout type="info">Mutually exclusive with [parameter count].</Callout>
...
elements are either the result of the [parse callback], the [choices or mapping] or the string
parameters themselves. Its value may be normalized according to the constraints specified in the
option definition. It has the following sets of attributes:
...
If specified, the option will accept a single parameter on the command-line, not multiple parameters
(as is the default). It can be either a string or a regular expression.

<Callout type="info">
  When using this attribute, [inline parameters] will be allowed for an array-valued option, unless
  explicitly disabled by the [inline] attribute.
</Callout>
...
<Callout type="info">Mutually exclusive with [parse callback].</Callout>
...
The **function** option is a non-niladic equivalent of the [flag] option. It exists solely for the
purpose of being configured with an expected parameter count. It is the only option that can accept
between zero and one parameter. It has the following sets of attributes:
...
The `paramCount` attribute. If present, it specifies how many parameters the option expects. It can
...
<Callout type="info">
  ...
  array-valued options, otherwise they would conflict with the semantics of the parameter count.
</Callout>

<Callout type="warning">
  If the parameter comes from an environment [data source], then the parameter received by the
  [parse callback] will contain a single element, which may be _less_ than the expected count. You
  can inspect the `index` property of the [argument sequence], which will be `NaN{:ts}` in this case.
</Callout>
