...
neither on the command-line nor as an [environment variable]. It is only evaluated at the end of the
parsing loop.
...
option value. This normalization is applied _before_ checking the [count limit] constraint.

<Callout type="info">
  Due to the nature of JavaScript's [Set] type, the order of array elements is preserved (i.e., the
...
Some attributes are shared by all valued option types except unknown-valued options. They are
described below.
