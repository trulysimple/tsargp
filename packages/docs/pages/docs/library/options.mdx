...
- `req.one` - an expression that is satisfied when _at least one_ requirement is satisfied
...
match. Any parameter whose normalized value that does _not_ match the regular expression will cause
...
The `range` attribute, if present, specifies a (closed) numeric range within which number parameters
...
specified, the option will accept a single parameter[^3] on the command-line, not multiple
...
<Callout type="info">
  Due to the nature of JavaScript's [Set] type, the order of elements is preserved (i.e., they
  reflect the order in which they were parsed).
</Callout>
...
This option has the following attributes.
...
message. It has the following attributes.
...
In addition to the set of [value attributes] and [parameter attributes], this option has the
...
In addition to the set of [value attributes], this option has the following attributes.
...
`yes`, `on`). They are also used by the [completion algorithm].

#### Falsity names

The `falsityNames` attribute, if present, specifies the names of the boolean falsity value. (e.g.
`false`, `no`, `off`). They are also used by the [completion algorithm].
