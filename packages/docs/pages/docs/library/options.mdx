...
In the case of an option key, the referenced option must also be present on the command-line. When
an object is specified, every referenced option must have the corresponding value, which can be any
value accepted by that option, or `undefined{:ts}` to signify _presence_ or `null{:ts}` to signify
_absence_.
...
both [inline parameters] and the [inline] attribute.
