...
and possibly appended with an ellipsis `...{:ts}` (in the case of variadic array options). Mutually
exclusive with [example value](#example-value).
...
- groups section - with `'Options'{:ts}` as default group heading - appending a colon to headings
