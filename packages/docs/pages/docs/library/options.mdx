...
_monadic_, that it accepts a single parameter; and _variadic_, variable number parameters (possibly
...
The `fallback` attribute, if present, specifies a value that is used in case the option is specified
...
<Callout type="info">
  This attribute is inherently different from the [default value] (they are not mutually exclusive).
</Callout>
...
those options whose names or synopsis match the regex `/flag/i{:ts}`.

If multiple patterns are provided, any matched one will suffice to include an option in the message.
...
ignores it during bash completion.
