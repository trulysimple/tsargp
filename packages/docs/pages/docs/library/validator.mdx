...
- `emptyVersionDefinition` - raised by the validator when a version option has an empty version string
...
- `emptyEnumsDefinition` - raised by the validator when an option has a zero-length enumeration array
...
- `deprecatedOption` - warning saved by the parser when a deprecated option is specified on the
  command-line
- `unsatisfiedCondRequirement` - raised by the parser when a conditional option requirement is not satisfied
