...
- **Enums constraint violation** -
  If the [enums] attribute is present, then the default, example and fallback values must, after
  being normalized, equal one of the enumerated values.
...
- `invalidClusterOption` -
  when either a variadic option or a command option is specified in the middle of a cluster argument
- `invalidBooleanParameter` -
  when a parameter to a boolean option fails to be converted
...
- `emptyEnumsDefinition` -
  when an option has a zero-length enumeration array
- `duplicateOptionName` -
  when an option has a duplicate name
- `duplicatePositionalOption` -
  when there are two or more positional options
- `duplicateEnumValue` -
  when a string or number enumeration constraint has duplicate values
...
- `enumsConstraintViolation` -
  when a value fails to satisfy either a string or a number enumeration constraint
...
- `mixedNamingConvention` -
  when a [name slot] contains names with different naming
  conventions
...
| enumsConstraintViolation   | `%o` = the option's key or specified name; `%s1`/`%n1` = the specified value; `%s2`/`%n2` = the enum values |
...
The `connectives` property specifies the connective words that are used to format option
