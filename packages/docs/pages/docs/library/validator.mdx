...
<Callout type="info">
  ...
  [negation names] and the [positional marker] are included in the validation.
</Callout>
...
- `stringEnumsConstraintViolation` -
  raised by both the parser or validator when a value fails to satisfy a string option's enumeration
  constraint
...
- `numberEnumsConstraintViolation` -
  raised by both the parser or validator when a value fails to satisfy a number option's enumeration
  constraint
