...
- **Option with no name** - If the option is not a positional one (i.e., it does not accept
  positional arguments), then it must contain at least one _non-empty_ name.
...
- **Duplicate cluster letter** - An option should not have duplicate cluster letters, and there
  cannot be two options with the same cluster letter.

<Callout type="info">
  In any of these, empty strings and `null{:ts}`s are ignored. Flag options' [negation
  names](options.mdx#negation-names) and the [positional marker](options.mdx#positional--marker) are
...
- **Zero enum values** - There may not be an option with zero enumerated values. The enumeration
  array should have _non-zero_ length.
- **Duplicate enum value** - There may not be an option with duplicate enumerated values. The
...
- **Invalid required option** - An option must not declare a requirement that references a
  _non-valued_ option.
- **Incompatible required value data type** - Since a requirement may be an object with values of
  any kind, the data type of these values must be validated at run-time. An option must not declare
  a requirement that includes an option key referencing a _non-niladic_ option, if it includes a
  required value that does _not_ conform to the data type expected by that option.
...
- **Duplicate positional optionn** - Since positional arguments have no name, they must pertain to
...
- **Empty positional markern** - If an option is declared with a positional marker, this marker
  cannot be the _empty_ string.
...
- `invalidOptionInCluster` - raised by the parser when either a variadic array option or a command
...
- `invalidOptionInCluster` - `'Option %o must be the last in a cluster.'{:ts}`
...
| invalidOptionInCluster    | `%o` = the specified option letter                                              |
