...
- **Zero enum values** -
  In string- and number-valued options, if the [enums] attribute is present, it must _not_ be empty.
  It should have non-zero length.
- **Duplicate enum value** -
  In string- and number-valued options, if the [enums] attribute is present, it must _not_ contain
  duplicate elements.
...
- **Enums constraint violation** -
  If the [enums] attribute is present, then the default, example and fallback values must equal one
  of the enumerated values after being normalized. (The latter are _not_ normalized.)
- **Regex constraint violation** -
  In a string-valued option, if the [regex] attribute is present, then the default, example and
  fallback values must match the regular expression after being normalized.
- **Range constraint violation** -
  In a number-valued option, if the [range] attribute is present, then the default, example and
  fallback values must be within this range after being normalized.
- **Limit constraint violation** -
  In an array-valued option, if the [limit] attribute is present, then the default, example and
  fallback values must have a length that does not exceed the given limit after being normalized.
...
- **Incompatible required value** -
  Since a requirement can be an object with values of any kind, their data type must be validated at
  run-time. Thus, an option must not declare a requirement that references a known-valued option, if
  it requires a value that does _not_ conform to the data type expected by that option.
...
- `invalidRequiredOption` -
  raised by the validator when an option references either a non-valued or an unknown-valued option
  in a requirement
- `invalidRequiredValue` -
  raised by the validator when an option uses a nullish value in a requirement referencing an option
  that is either always required or has a default value
...
following optional properties, which are enumerators from `ErrorItem`:
...
- `invalidVersionDefinition` - `'Option %o contains empty version.'{:ts}`
