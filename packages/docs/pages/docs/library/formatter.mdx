...
- `progName` - the name of the program to use in usage sections
...
- `indent` - the level of indentation of the section content (defaults to `0{:ts}`)
- `filter` - a list of options keys to include or exclude (defaults to none)
...
- `style` - the style of the default group heading (defaults to `tf.bold{:ts}`)
