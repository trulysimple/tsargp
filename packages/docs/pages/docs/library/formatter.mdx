...
- `separator` - `'Values can be delimited with (%s|%r).'{:ts}`
- `paramCount` - `'Accepts (multiple|%n|at most %n|at least %n|between %n) parameters.'{:ts}`
- `positional` - `'Accepts positional arguments(| that may be preceded by %o).'{:ts}`
- `append` - `'Can be specified multiple times.'{:ts}`
