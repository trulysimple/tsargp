...
receives a "slot" in the column, and the name is left-aligned within that slot. See [names column]
...
The `filter` property specifies a list of patterns that can be used to select the options that will
appear in the help message. The filter matches options' names, synopsis and environment variable
names. If multiple patterns are provided, any matched one will suffice to include an option in the
message.
...
Help messages are styled with the same set of styles as [error messages], except that they may get
overriden by (or combined with) an option's [display styles], if present.
