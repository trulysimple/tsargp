...
can be printed in a terminal. Visually, it can be divided into the following pieces of content:
...
There are different ways of obtaining help messages, as described below.

### Default help

The `formatHelp` method returns a message corresponding to the _default_ option group, which is the
set of options that did not explicitly declare an [option group] in their definition. It accepts no
parameter.
...
Except for example values, the parameter is appended with an ellipsis `'...'{:ts}` if the option
...
following optional properties, which are enumerators from `HelpItem`:
...
| Error          | Specifiers                               |
| -------------- | ---------------------------------------- |
| synopsis       | `%t` = the option's synopsis             |
...
that they may get overriden by (or combined with) an option's [display styles], if present.
...
[deprecated]: options#synopsis--deprecated
