...
The `filters` property specifies a list of regular expressions that can be used to filter the
options that will appear in the help message. The filter matches an option's names, synopsis and
environment variable name, if any. If multiple patterns are provided, any matched one will suffice
to include an option in the message.
