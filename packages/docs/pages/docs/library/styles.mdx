...
style of number values is `fg.yellow{:ts}`, then any number value that appears in the option
...
Text is split on paragraphs, list items and word boundaries using a combination of simple regular
...
if you find some corner case that is not currently covered by unit tests, please submit a [bug
report].
...
<Callout type="warning">
  ...
  ``` `Requires ${style(tf.bold)}%v`{:ts} ```. You must insert a space between them.
</Callout>
...
When wrapping text to a desired width, the terminal string will try to respect this setting, but if
the desired width does not accommodate the length of the largest word in the text while respecting
its indentation level, the text will instead be wrapped relative to the beginning of each line.
...
The `CompletionMessage` class represents a list of completion words. It inherits from
`Array<string>{:ts}` and, on Linux, produces a string with the words separated by line feeds. It
should be printed with `console.log` or equivalent.
