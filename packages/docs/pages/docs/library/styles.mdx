...
output is being redirected), the terminal string instance will avoid wrapping and will suppress
control sequences that would otherwise be used to render text in a terminal.
...
In addition, this class overrides the `toString` method and provides a `message` property, both of
which can be used to obtain a string wrapped with a default terminal width. This default depends on
the kind of message: in the case of `TerminalMessage`, it is `process.stdout.columns{:ts}` (the
width of the standard _output_ stream). Therefore, this kind of message should be printed with
...
<Callout type="default">
  Messages are produced by the library as instances of one of the classes described below. So you
  can check what kind of message a captured error is through `instanceof{:ts}`.
</Callout>
