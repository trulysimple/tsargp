...
During splitting, format specifiers prefixed with a percent sign `%` are extracted from the text and
processed by a formatting callback. This is used by the library in different places:
...
The `CompletionMessage` class represents a list of completion words. It inherits from a string array
and, on Linux, produces a string with the words separated by line feeds. It should be printed with
