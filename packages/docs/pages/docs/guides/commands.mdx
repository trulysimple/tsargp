...
be displayed in the help message. This option is optional and defaults to an empty array.

Now let's define a similar option for the parameters of a dual-argument operation. It has the same
definition as above, with additional constraints: it is required and accepts at most _two_ values.
